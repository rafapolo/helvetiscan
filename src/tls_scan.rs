...
fn writer_loop_tls(
    ...
    let conn = crate::shared::open_db(&db_path)
        .with_context(|| format!("tls writer: open db {:?}", db_path))?;
    ...
    while let Some(row) = result_rx.blocking_recv() {
        ...
        if batch.len() >= batch_size {
            if let Err(e) = flush_tls_batch(&conn, &mut batch) {
                crate::shared::append_error_log(&db_path, &format!("tls flush_batch: {e:#}"));
                return Err(e);
            }
        }
    }
    if !batch.is_empty() {
        if let Err(e) = flush_tls_batch(&conn, &mut batch) {
            crate::shared::append_error_log(&db_path, &format!("tls flush_batch (final): {e:#}"));
            return Err(e);
        }
    }
    let _ = done_tx.send(());
