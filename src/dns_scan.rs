...
pub(crate) async fn cmd_dns(
    ...
    let (progress, own_progress) = match ext_progress {
        ...
        None => (Arc::new(Progress::new(pending.len() as u64, "resolved", "errors")), true),
    };
...
fn writer_loop_dns(
    ...
    while let Some(row) = result_rx.blocking_recv() {
        ...
        if batch.len() >= batch_size {
            if let Err(e) = flush_dns_and_email_batch(&conn, &mut batch) {
                crate::shared::append_error_log(&db_path, &format!("dns flush_batch: {e:#}"));
                return Err(e);
            }
        }
    }
    if !batch.is_empty() {
        if let Err(e) = flush_dns_and_email_batch(&conn, &mut batch) {
            crate::shared::append_error_log(&db_path, &format!("dns flush_batch (final): {e:#}"));
            return Err(e);
        }
    }
    let _ = done_tx.send(());
